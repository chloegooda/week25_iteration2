// FOREST LEVEL classes

// TODO fix flipx, spotlight, and warp shaders

class BaseForest extends Phaser.Scene {
	constructor(id) {
		super(id);
		this.id = id;
		this.tileDataKey;
	...
	preload() {
		...
		this.load.image('forest-tileset', 'assets/forest-tileset-extruded.png');
		// player
    ...
    create() {
		...
		// map
		var background = this.add.image(config.width / 2, config.height / 2, "forest-background").setScale(1.4, 1).setScrollFactor(0);
		var midground = this.add.image(config.width / 2, config.height / 2, "forest-midground").setScale(1.6, 1.6).setScrollFactor(0.5);
		this.map = this.make.tilemap({ key: this.tileDataKey });
		var tileset = this.map.addTilesetImage('tileset', 'forest-tileset', 16, 16, 1, 2);
	...
	createButtons() {
		...
		restartButton.on('pointerdown', function() {
			if (paused) {
				...
				if (this.id === 'forestA') {
					forestRestartsA++;
					trackEvent('Restart', 'ForestRestart', forestRestartsA);
				}
				else if (this.id === 'forestB') {
					forestRestartsB++;
					trackEvent('Restart', 'ForestRestart', forestRestartsB);
				}
	...
	switchScene() {
		switch(this.id) {
            case 'forestA':
                completeLevels.forest[0] = true;
				trackEvent('LevelComplete', 'Forest 1');
				this.scene.start('forestB');
				break;
            case 'forestB':
                completeLevels.forest[1] = true;
				trackEvent('LevelComplete', 'Forest 2');
...
class ForestA extends BaseForest {
	constructor() {
		super('forestA');
		this.tileDataKey = 'forest1';
		this.tileDataSource = 'assets/forest1.json';
	}
...
class ForestB extends BaseForest {
	constructor() {
        super('forestB');
        this.tileDataKey = 'forest2';
        this.tileDataSource = 'assets/forest2.json';
	}
